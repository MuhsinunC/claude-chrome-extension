...
**Step B — install a `globalThis.fetch` override, synchronously at script run.** The SDK resolves `fetch` lexically at call time, so an override installed before the deferred modules is guaranteed to be used. The override reads current config from `chrome.storage.local` **at intercept time** (fetch is async anyway), so there is no startup race and settings changes take effect without reload. (No XHR override: the SDK is fetch-based — confirmed no `XMLHttpRequest` in the client bundle — and streaming is `ReadableStream` over fetch, preserved by forwarding the body with `duplex: 'half'`.) Behavior:
...
2. **Reroute the whole Anthropic API surface** — any `https://api.anthropic.com/v1/*` path (messages, complete, count_tokens, AND models/files/skills, which the SDK also calls): rewrite origin to `customApiBaseUrl`; set `x-api-key: <customApiKey>` and delete any `Authorization: Bearer`. Rerouting all of `/v1/*` (not just messages) ensures the key never leaks to `api.anthropic.com` when a custom base is set, and lets a proxy serve every endpoint. Required because the SDK's `baseURL` arg is already truthy so a config-based fallback never fires; the network layer is the only injection-only place to reroute. Keep `anthropic-version`; add `anthropic-dangerous-direct-browser-access: true`.
...
Freshly extracted from the Web Store (Chrome/140 stable; CRX 5.26 MB; `manifest.json` version `1.0.74`, name `Claude`). Reproducibility note (deliberate deferral): this was an extraction validated on 2026-05-31 on a personal repo; the pristine v1.0.74 bytes are committed to the `upstream` branch during the restructure (§5 / §6 step 8), after which this section is pinned to that commit SHA so the validation below is independently reproducible rather than a point-in-time assertion. **Pinned: `upstream` @ `860814f`** (`git show 860814f:manifest.json`).
