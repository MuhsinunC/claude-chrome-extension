...
import { i as c } from './SentryService-Bpayo6Ny.js';
import {
  L as d,
  t as u,
  M as m,
  ...
  P as h,
  Q as w,
} from './mcpPermissions-juDHLu3j.js';
import './client-BLU1RtqI.js';
let y = null,
  f = null,
  _ = !1,
  g = !1,
  ...
  I = null;
function v(e) {
  e?.includes('native messaging host not found') && (g = !1);
}
async function E() {
  try {
    return await (async function () {
      ...
      try {
          ...
          try {
            ...
            if (
              ...
              return (
                ...
                (f = n.name),
                (g = !0),
                y.onMessage.addListener(async (e) => {
                  await N(e);
                }),
                y.onDisconnect.addListener(() => {
                  const a = chrome.runtime.lastError?.message;
                  ((y = null), (f = null), (T = !1), e(t.MCP_CONNECTED, !1), v(a), d());
                }),
                y.postMessage({ type: 'get_status' }),
                !0
              );
            a.disconnect();
          } catch (a) {}
        ...
        return (a instanceof Error && v(a.message), !1);
      } finally {
        _ = !1;
      }
    })();
    ...
    case 'tool_request':
      await (async function (e) {
        try {
          ...
          if ('execute_tool' === t) {
            if (!a?.tool) return void P(m('No tool specified'));
            const e = a.client_id,
              t = a.args?.tabGroupId,
          ...
          } else P({ content: `Unknown method: ${t}` });
        } catch (t) {
          P(m(`Tool execution failed: ${t instanceof Error ? t.message : 'Unknown error'}`));
        }
      })(a);
      break;
    case 'status_response':
      b &&
        (clearTimeout(I), (I = null), b({ nativeHostInstalled: g, mcpConnected: T }), (b = null));
      break;
    case 'mcp_connected':
      !(async function () {
        ((T = !0), e(t.MCP_CONNECTED, !0), await u.initialize(), u.startTabGroupChangeListener());
      })();
      break;
    case 'mcp_disconnected':
      ((T = !1), e(t.MCP_CONNECTED, !1), u.stopTabGroupChangeListener());
  }
}
function P({ content: e, is_error: t }, a) {
  if (!y) return;
...
async function k(e, t) {
  try {
    ...
    if (!a.pathname.startsWith(L)) return !1;
    const s = a.pathname.substring(8).toLowerCase();
    if ('reconnect' === s)
      return (
        await (async function (e) {
          try {
            (await A(), await new Promise((e) => setTimeout(e, 500)));
            const e = await E();
            p('claude_chrome.extension_url.reconnect', { success: e });
          } catch (t) {
    ...
    if (s.startsWith('tab/')) {
      ...
      return (
        await (async function (e, t) {
          if (isNaN(e))
            return (
              p('claude_chrome.extension_url.tab_switch', { success: !1, error: 'invalid_tab_id' }),
              await M(t),
              !0
            );
          try {
            await u.initialize();
            const a = await u.findGroupByTab(e);
            if (!a || a.isUnmanaged)
              return (
            ...
            return (p('claude_chrome.extension_url.tab_switch', { success: !1 }), await M(t), !0);
          }
        })(e, t),
        !0
      );
...
async function C() {
  try {
    const t = (await i.getAllPrompts()).filter((e) => e.repeatType && 'none' !== e.repeatType);
...
async function O(e) {
  (chrome.sidePanel.setOptions({
    ...
    await u.initialize(!0));
  const t = await u.findGroupByTab(e);
  if (t) {
    if (t.isUnmanaged) {
      try {
        await u.adoptOrphanedGroup(e, t.chromeGroupId);
      } catch (a) {}
      return;
    }
  } else {
    try {
      await u.createGroup(e);
    } catch (a) {}
    E();
  }
}
...
async function D(a, s) {
  const n = `session_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`,
    r = await chrome.windows.create({ url: a.url || 'about:blank', type: 'normal', focused: !0 });
  if (!r || !r.id || !r.tabs || 0 === r.tabs.length)
    throw new Error('Failed to create window for scheduled task');
  const o = r.tabs[0];
  if (!o.id) throw new Error('Failed to get tab in new window for scheduled task');
  await u.initialize(!0);
  await u.createGroup(o.id);
  (await e(t.TARGET_TAB_ID, o.id),
    ...
    await (async function (e) {
      const { tabId: t, prompt: a, taskName: s, runLogId: n, sessionId: r, isScheduledTask: o } = e;
...
(chrome.runtime.onInstalled.addListener(async (e) => {
  ...
  chrome.runtime.onStartup.addListener(async () => {
    (h(), await S(), await u.initialize(), E(), await C());
  }),
  chrome.permissions.onAdded.addListener((e) => {
    e.permissions?.includes('nativeMessaging') && E();
  }),
  chrome.permissions.onRemoved.addListener((e) => {
    e.permissions?.includes('nativeMessaging') && A();
  }),
    ...
    (a, s, r) => (
      (async () => {
        if ('PLAY_NOTIFICATION_SOUND' !== a.type) {
              ...
              const e = async (t = 0) => {
                try {
                  ...
                  (await new Promise((t) => setTimeout(t, e)),
                    await new Promise((e, t) => {
                      chrome.runtime.sendMessage(
                        { type: 'LOAD_CONVERSATION', conversationUuid: a.conversationUuid },
                        () => {
                          chrome.runtime.lastError
                            ? t(new Error(chrome.runtime.lastError.message))
                            : e();
          ...
          if ('logout' === a.type)
            try {
              (await n(), await u.clearAllGroups(), await w(), r({ success: !0 }));
            } catch (o) {}
                ...
                ? (I && clearTimeout(I),
                  new Promise((e) => {
                    ((b = e),
                      y.postMessage({ type: 'get_status' }),
                      (I = setTimeout(() => {
                        ((b = null), e({ nativeHostInstalled: g, mcpConnected: T }));
                      }, 1e4)));
                  }))
                : { nativeHostInstalled: g, mcpConnected: T };
            })();
            r({ status: e });
            ...
            try {
              ...
              const s = chrome.runtime.getURL('options.html'),
                n = (await chrome.tabs.query({})).find((e) => e.url?.startsWith(s));
              (n && n.id
            ...
            if ('CURRENT_TAB' === a.fromTabId && s.tab?.id) {
              e = (await u.getMainTabId(s.tab.id)) || s.tab.id;
            } else 'number' == typeof a.fromTabId && (e = a.fromTabId);
            (e && chrome.runtime.sendMessage({ type: 'STOP_AGENT', targetTabId: e }),
              r({ success: !0 }));
          } else if ('SWITCH_TO_MAIN_TAB' === a.type)
            if (s.tab?.id)
              try {
                await u.initialize(!0);
                const e = await u.getMainTabId(s.tab.id);
                if (e) {
                  await chrome.tabs.update(e, { active: !0 });
                  ...
                  ? (async () => {
                      ...
                      if (e)
                        try {
                          ...
                          if (await u.findGroupByTab(e)) return void r({ success: !0 });
                          const a = await chrome.tabs.query({ groupId: t }),
                            s = async (t) => {
                              if (t >= a.length) return void r({ success: !1 });
                              const n = a[t];
                              if (n.id === e || !n.id) return void (await s(t + 1));
                              const o = n.id,
                                i = Date.now(),
                                c = R.get(o);
                              if (c && i - c.timestamp < 3e3)
                                return c.isAlive ? void r({ success: !0 }) : void (await s(t + 1));
                              chrome.runtime.sendMessage(
                                {
                                  type: 'MAIN_TAB_ACK_REQUEST',
                                  secondaryTabId: e,
                      ...
                      if (e)
                        try {
                          ...
                          if (void 0 === t || t === chrome.tabGroups.TAB_GROUP_ID_NONE)
                            return void r({ success: !1 });
                          (await u.initialize(),
                            await u.dismissStaticIndicatorsForGroup(t),
                            r({ success: !0 }));
                        } catch (o) {
                          r({ success: !1 });
                        }
  ...
  chrome.tabs.onRemoved.addListener(async (e) => {
    await u.handleTabClosed(e);
  }),
  chrome.webNavigation.onBeforeNavigate.addListener(async (e) => {
    0 === e.frameId && (await k(e.url, e.tabId));
  }),
      ...
      try {
        ...
        if (i) {
          ...
          if ('monthly' === i.repeatType || 'annually' === i.repeatType)
            try {
              const { SavedPromptsService: e } = await r(async () => {
                const { SavedPromptsService: e } = await import('./permissions-BsMI2TUG.js').then(
                  (e) => e.N
                );
